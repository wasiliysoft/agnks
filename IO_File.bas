...
Private Sub init_agnksConfig()
    ...
    Else
        ...
        Close #fh
    End If
    frmStart.lblPC.Caption = Format(agnksÑonfig.PC, "0.0000")
    frmStart.Price.Caption = Format(agnksÑonfig.Price, "0.00")  ' Âêëàäêà Ñõåìà
    frmStart.lbl_Price.Caption = Format(agnksÑonfig.Price, "0.00") ' Âêëàäêà Ñèñòåìíûå
    frmStart.lbl_gnPlot.Caption = Format(pAgnksÑonfig.plot, "0.0000")
End Sub
...
Sub updatePrice()
    Dim d As Double: d = 0
    Dim sInput As String
    sInput = InputBox("Ââåäèòå íîâîå çíà÷åíèå öåíû ãàçà", , Format(agnksÑonfig.Price, "0.00"))
    If (Len(sInput) = 0) Then Exit Sub
    On Error Resume Next
        d = CDbl(sInput)
    On Error GoTo 0
